...
<ComponentDocsHeader
  ...
  componentName="useCloseDropDown"
  summary="Used to set the closed state of an open and close UI"
  versionAdded="1.1.0"
  versionLatest="1.5.2"
/>
...
The `ref` argument is used in Chakra's `useOutsideClick` function,
used internally, to "close" a component. The `ref` allows the function to know
where "outside the component" is.
