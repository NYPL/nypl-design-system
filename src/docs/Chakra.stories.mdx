...
DS specified CSS variables are still exported, but we will remove those in
favor of the CSS variables provided by Chakra for ease of use.
...
const Button = {
  baseStyle: {
    py: 2, // var(--nypl-space-xs) which results in a value of 8px or 0.5rem
    px: 4, // var(--nypl-space-s) which results in a value of 16px or 1rem
    // ... other styles
  },
  sizes: { ... },
  variants: { ... },
