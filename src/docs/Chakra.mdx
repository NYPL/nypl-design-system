...
and components based on the Reservoir Design System.

There are situations, however, where you may want to disable these global styles.
For example, the NYPL `Header` app is a DS-based application that is rendered on
top of other applications and websites. These digital products _may_ not be built
with the DS and therefore, the global base styles of the application may conflict
with the styles of the DS.
