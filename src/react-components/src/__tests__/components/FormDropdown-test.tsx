...
describe("FormDropdown", () => {
  ...
  it("if both labelId and ariaLabel are defined, select has aria-labeledBy", () => {
    wrapper = Enzyme.shallow(<FormDropdown labelId="label" ariaLabel="aria-label" isRequired={false} dropdownId="hi" options={["opt1", "opt2", "opt3"]} onSelectChange={changeCallback} onSelectBlur={blurCallback} />);
    expect(wrapper.find("select").props()["aria-labelledby"]).to.equal("label");
  });

  it("if only aria-label is defined, select has aria-label", () => {
    wrapper = Enzyme.shallow(<FormDropdown ariaLabel="aria-label" isRequired={false} dropdownId="hi" options={["opt1", "opt2", "opt3"]} onSelectChange={changeCallback} onSelectBlur={blurCallback} />);
    expect(wrapper.find("select").props()["aria-label"]).to.equal("aria-label");
  });
