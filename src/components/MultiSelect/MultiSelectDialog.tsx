...
export const MultiSelectDialog = chakra(
  forwardRef<HTMLDivElement, React.PropsWithChildren<MultiSelectDialogProps>>(
    (props, ref) => {
      ...
      const [defaultChildCheckboxes, setDefaultChildCheckboxes] = useState("none");
      ...
      const isAllChecked = (
        ...
        targetItem: MultiSelectItem
          ...
          const childIds: string[] = targetItem.children.map((childItem) => childItem.id);
          if (childIds.length > 0 && selectedItems[multiSelectId] !== undefined) {
            return childIds.every((childItem) => selectedItems[multiSelectId].items.includes(childItem)
            );
          }
          return false;
      };
      ...
      const showViewLabel = () => {
        if (itemsList.length > 0) {
          return <Text onClick={ () => viewAllItems() } __css={styles.viewAllButton}
            id="text-btn" marginTop="s" > 
            {viewAllLabel}
          </Text>
        }
      }
      ...
      const itemsNotFound = () => {
        if (itemsList.length === 0) {
        return <Text id="text-btn" marginTop="xs" isItalic="true"> 
          No options found
        </Text>
        }
      }
      ...
      const viewAllItems = () => {
        ...
        setListItemsCount(listItemsCount === defaultItemsVisible ? items.length : defaultItemsVisible)
        if (listItemsCount === defaultItemsVisible) {
          setItemsList(items);
        } else {
          displayDefaultItems();
      ...
      const buttonTextLabel = () => {
        return <Text __css={styles.accordianLabel} title={buttonText}>{truncateText(buttonText as string, 20)}</Text>
      }
      ...
      return (
        ...
        >
          ...
          />
          <Accordion
            sx={{ button: styles.menuButton, "> div": {
              _first: {
                "> div": { 
                  _first: {
                    minWidth: "200px",
