...
<Banner
  content={
    <>
      <strong>IMPORTANT:</strong> The `imageProps` image is not the same as the
      `backgroundImageSrc` image. For a full list of what props to use for each
      variant, check the <a href="#active-variants">Active Variants</a> section.
    </>
  }
  mt="s"
  mb="s"
...
<Banner
  content={
    <>
      ...
      CSS `background-image` property. This means that more work is needed to
      set a fallback for the background image. See example below.
    </>
  }
  mt="s"
  mb="s"
