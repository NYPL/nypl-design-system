...
For the `campaign` variant, the component background can be set through the
`backdropBackgroundColor` or `backgroundImageSrc` prop. When both are set, the
`backgroundImageSrc` will take precedence. For clarification, the component
background is sometimes referred to as the "backdrop."

The `campaign` variant offers additional props to configure the background.

- `backdropBackgroundColor`
- `isDarkBackgroundImage`
- `isBlurredBackgroundImage`<br /><br />
...
##### isDarkBackgroundImage and isBlurredBackgroundImage

The `isDarkBackgroundImage` and `isBlurredBackgroundImage` props are used to
control the color and blur treatment of the background image set using the
`backgroundImageSrc` prop.
