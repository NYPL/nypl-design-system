...
`Hero` composes a Chakra `Box` component, so you may pass
...
The `"primary"` hero variant can be used with the `backgroundColor`, `backgroundImageSrc`,
...
The `"tertiary"` hero variant can be used with the `backgroundColor`, `foregroundColor`,
...
The `"campaign"` hero variant can be used with the `backdropBackgroundColor`,
