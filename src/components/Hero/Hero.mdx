...
Note: the `imageProps` image is not the same as the `backgroundImageSrc` image.
For a full list of what props to use for each variant, check the
[Active Variants](#active-variants) section.
...
However, the background image is rendered through the CSS `background-image`
property. This means that more work is needed to set a fallback for the
background image.
