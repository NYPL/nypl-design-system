...
The Reservoir handles the grouping of the two `<input>` elements in the "date
range" mode by wrapping the elements in a `<fieldset>` element with its own
`<legend>` label for the group. Note that this is in addition to the two labels
that each `<input>` element is associated with.
