...
to the `CheckboxGroup` and will automatically be applied to all children `Checkbox`es.
If you are using the `Checkbox` in isolation, you will need to pass the `name` directly
to the `Checkbox`. The `value` attribute is optional, but is recommended because it is
needed by the `CheckboxGroup`'s `onChange` function.
