...
- {<Link href="#additional-attributes" target="_self">Additional Attributes</Link>}
...
Beyond the `Checkbox` props, you may also pass `name` and `value` attributes.

In fact, a `name` attribute is required for a checkbox to be submitted as form data
(though `name="default"` will automatically be applied if none is passed). If you
are using the `CheckboxGroup` wrapper (recommended) a `name` must be passed to the
`CheckboxGroup` and will be applied to all children `Checkbox`es.

The `value` attribute is optional, but is recommended because it is needed by the
`CheckboxGroup`'s `onChange` function.
