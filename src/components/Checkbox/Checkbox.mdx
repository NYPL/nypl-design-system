...
- {<Link href="#checked" target="_self">Checked</Link>}
- {<Link href="#indeterminate" target="_self">Indeterminate</Link>}
- {<Link href="#additional-states" target="_self">Additional States</Link>}
- {<Link href="#name-and-value-attributes" target="_self">Name and Value Attributes</Link>}
...
For a checked `Checkbox` to be submitted as form data, a `name` attribute is required.
If you are using the `CheckboxGroup` wrapper (recommended) this `name` must be passed
to the `CheckboxGroup` and will be applied to all children `Checkbox`es. If you are
using the `Checkbox` in isolation, you should pass the `name` directly to the `Checkbox`
(or a `name` of `"default"` will automatically be applied). The `value` attribute is
optional, but is recommended because it is needed by the `CheckboxGroup`'s `onChange`
function.

<Source
  code={`
// If checked, this would add library=SASB to form data
<Checkbox name="library" value="SASB"/> 
`}
  language="tsx"
/>
