...
An NYPL privacy policy link will always render on every screen and the link
will open in a new tab.

**Note**: For the purposes of Storybook, only one (1) `FeedbackBox` component
example is rendered on the page. The `FeedbackBox` example below alternately
...
`TextInput` component. Each of these components have their own accessibility
...
The "confirmation" screen will render a check icon, a success message, an
additional confirmation text through the `confirmationText` prop, the privacy
policy link, and a "Return to Browsing" button. When rendered, focus will be
set to the confirmation message.
