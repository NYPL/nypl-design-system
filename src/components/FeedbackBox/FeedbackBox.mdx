...
**Note:** For the purposes of Storybook, only one `FeedbackBox` component
example is rendered on the bottom right of this page.

- The `FeedbackBox` example alternately renders the "confirmation" and "error"
  screens on each form submission. This is just to demonstrate the different
  states of the component. In practice, the consuming app is responsible for
...
- The background color and color mode can be updated in the `FeedbackBox`
  ...
  Note the accessible focused state of the button on different backgrounds.
...
information above the form's description text. The text or TSX element passed
