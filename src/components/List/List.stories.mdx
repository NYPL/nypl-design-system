...
- [Definition List of Links](#definition-list-of-links)
...
ordered, and definition lists. This includes `ul`, `ol`, and `dl` elements.
...
`dt` and `dd` elements for definition lists.

The definition type for the `List` component is wrapped in a `section` element
...
```jsx
// With `dt`/`dd` elements
<List type={ListTypes.Definition} title="Animal Crossing Fish">
  <dt>Mahi-mahi</dt>
