...
The NYPL standard column count is `3`, so the `SimpleGrid` component is set to use 3 columns by default and the `columns` props is optional.
...
for (const icon in IconNames) {
  ...
  const iconRender = (
    <Icon decorative name={icon} size={IconSizes.xxxlarge} titleText={iname} />
  );
