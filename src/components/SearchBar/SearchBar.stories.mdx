    ...
    {args => (
      <>
        <SearchBar {...args}>
          ...
          <Input
            ariaLabelledBy="button-optional-pattern"
            id="input2"
            placeholder="Item Search"
            required={true}
            type={InputTypes.text}
            attributes={{
              ["aria-describedby"]: "optional-helperText",
            }}
          />
          <Button
            buttonType={ButtonTypes.Primary}
            id="button-optional-pattern"
            type="submit"
          >
            <Icon
              name={IconNames.search}
        ...
        <HelperErrorText id="optional-helperText" isError={false}>
          Search for items in Animal Crossing New Horizons
        </HelperErrorText>
      </>
    )}
