...
The main wrapper element that is rendered is a `<form>` DOM element. You are
expected to pass an `action` attribute to specify where the search query should
be sent. You may also pass a `method` attribute, although search forms generally
use `method="get"`, which is already the default behavior.
