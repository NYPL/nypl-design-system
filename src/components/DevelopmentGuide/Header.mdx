...
that gets embedded on said digital applications. This complex component implements
the Reservoir Design System and contains features for logging in, logging out,
searching, and navigating the NYPL.org site.
...
The `Header` can be embedded on an NYPL app through the following code snippet:
...
- Notification `<aside>` HTML element - This landmark element renders NYPL alert
  ...
  in the [Notification Accessibility](?path=/docs/components-feedback-notification--docs)
  section.
