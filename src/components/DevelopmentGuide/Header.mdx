...
- has a log in form for NYPL's [Catalog](https://nypl.na2.iiivega.com/)
  and [Research Catalog](https://www.nypl.org/research/research-catalog/),
- has a search form for the Catalog and across [NYPL.org](https://www.nypl.org/).
...
to mitigate the "jumping" effect that occurs when the `Header` is loaded asynchronously
on a page. Otherwise, once loaded, the main content will be pushed down.

NOTE: we cannot predict the length of the alert text in the banner (if any) and the page
may still jump, but not by much.

## Accessibility

The `Header` contains multiple native HTML elements that work together
to make the component accessible. It contains:

- Skip navigation
  - More information about this feature in the following section.
- Notification `<aside>` HTML element
  - This landmark element renders NYPL alert messages. Per accessibility guidelines,
    this element cannot be nested within the `<header>` element, so it is rendered above.
    More information can be found in the [Notification Accessibility](../?path=/docs/components-feedback-notification--docs#accessibility)
    section.
- `<header>` HTML element
  - This landmark element denotes the top content section of the page. There should only
    be one main `<header>` element on the page as a child of the `<body>` element.
- `<nav>` HTML elements
  - The `Header` component renders one (1) `<nav>` element in the mobile view and two (2)
    `<nav>` elements in the desktop view. Each `<nav>` element appropriately contains its own
    `aria-label` attribute to differentiate it from one another.
- `<form>` HTML element
  - The `<form>` element is used in the search feature.
- Dropdowns buttons with focus-trap feature
  - The `Header` uses `react-focus-lock` to trap (or loop) focus within its various dropdowns:
    the login submenu, the search form, and the mobile navigation submenu. This is to make
    keyboard navigation easier for users. Trapping focus means that when a parent element is
    clicked and its dropdown becomes visible, focus remains among its children while the user tabs.
    The user may close the dropdown and escape the focus trap by clicking on the parent element
    once again, or by using the escape key.
  - Note: This is how the original NYPL `Header` was implemented and its Reservoir counterpart
    replicates this behavior. However, this is not the _currently recommended_ way to implement
    focus management in dropdowns. According to the [W3C](https://www.w3.org/wiki/MenuButton),
    up and down arrow keys, rather than tab, should loop focus over child elements. Tabbing should
    instead close the dropdown and move focus to the following parent. In the future, we hope to
...
If using the Reservoir [`Template`](../?path=/docs/components-page-layout-template--docs)
components, we recommend adding the `id` of `"mainContent"` to the `TemplateMain`
component (which renders a `main` HTML element).

The second link in the skip navigation points to [NYPL's accessibility resource](https://www.nypl.org/accessibility).
