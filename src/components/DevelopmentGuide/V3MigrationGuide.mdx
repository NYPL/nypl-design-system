...
- {<Link href="#chakra-ui-2.8">Chakra UI 2.8</Link>}
...
- Due to some JS features and how we test, consuming apps need to add the
  following to their Jest setup file. They are needed for the following
  components and hooks: `Tabs`, `Slider`, and `useNYPLBreakpoints`.
  - We are looking into making updates so that consuming apps do not have to
    include these test helpers in future releases.

<Source
  code={`
// Added in Jest setup file as defined in jest.config.js
