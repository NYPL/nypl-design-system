...
When it comes to routing, the patterns set in Chakra's `Link` component can also
be used in the Design System's `Link` component. This means that the routing
library's component should be passed to the DS `Link` component in the `as` prop.
By default, an anchor `<a>` element is rendered.
