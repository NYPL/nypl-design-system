...
- [Visited state patterns](#visited-state-patterns)
- [All link variants](#all-link-variants)
...
The `Link` component should be passed an HTML `href` attribute and be used
for navigation. If an `onClick` action is required, a `Button` component
should be used instead. In simple terms, a link goes somewhere and a button
does something.
...
<Banner
  content={
    <>
      <strong>IMPORTANT:</strong> Links that look like buttons should still act
      like links and not act like buttons. Refer to the{" "}
      <a href="#accessibility">Accessibility</a> section above for
      clarification.
    </>
  }
  mt="s"
...
<Canvas of={LinkStories.AllLinkVariants} />

## Links with icons

To render an `Icon` as part of the link, make sure that the `Link` component
wraps the `Icon`. Use `variant="action"` to apply appropriate styling to
...
Following Next.js 13+ and Chakra's recommendations, pass the NextJS `Link`
component in the `as` prop in the DS `Link` component.
...
<Source
  ...
  // ...
  <Link as={NextLink} variant="action" href="#">
    Next Page
  </Link>
`}
  language="tsx"
...
<Banner
  content={
    <>
      <strong>IMPORTANT:</strong> NYPL uses Next.js for their web applications
      so while the following pattern is available, it has not been heavily
      tested.
    </>
  }
  mt="s"
...
<Source
  ...
  import { Link } from "@nypl/design-system-react-components";
  // ...
  <Link as={ReactRouterLink} variant="action" href="#">
    Next Page
  </Link>
`}
  language="tsx"
