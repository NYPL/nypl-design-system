...
- {<Link href="#all-link-variants" target="_self">All Link Variants</Link>}
...
as a child if no `href` was passed.
...
In DS 3.0.0+, this pattern is no longer supported. The `href` attribute should be
passed to render an accessible and valid link. There are cases where the `href`
attribute is not required, such as when using the component with an application
framework router. For more information and recommended routing patterns, see the
following sections.
