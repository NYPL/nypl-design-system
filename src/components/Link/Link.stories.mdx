...
`Link` component renders an `<a>` element with the `href` attribute. Many of the
`Link` variants have a visited state for accessibility and UX purposes.
...
As mentioned in the [Links Accessibility Guide](/story/accessibility-guide-links--page),
there is no explicit rule that dictates using an underline to indicate a link. The optional
`isUnderlined` prop is true by default, but can be set to false to remove the underline in
all browser states except for hover.
