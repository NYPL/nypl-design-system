...
<Meta
  ...
  argTypes={{
    ...
    color: {
      control: { type: "select" },
      table: { defaultValue: { summary: "LogoColors.UiBlack" } },
...
<Canvas withToolbar>
  ...
  >
    {(args) => (
      <Logo
        additionalStyles={
          args.color === "LogoColors.UiWhite"
            ? { background: "ui.gray.xdark", padding: "16px" }
            : undefined
        }
...
export const iconRow = (icon, opts = {}) => {
  ...
  return (
    <div style={styles} key={key}>
      ...
      <Logo name={LogoNames[icon]} size={size} color={color} />
    </div>
  );
};
export const logos = [];
...
for (const logo in LogoNames) {
  logos.push(iconRow(logo, { displayValue: `LogoNames.${logo}` }));
}
...
its logo name in the `title` element. The default text in the `title` element
...
<Canvas withToolbar>
  <Story name="All Display Icons">{allIconsType(logos)}</Story>
</Canvas>
