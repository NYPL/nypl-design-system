...
The `Logo`'s `aria-hidden` attribute is set to `true` by default so screenreaders
will ignore them. This is because most logos are considered decorative.

If your `Logo` is not purely decorative, pass `decorative={false}` and a
descriptive `title` for screenreaders to read. The default `title` is simply the
`svg` file name.
...
Update the size of the `Logo` by passing the `size` prop with a value from the
`LogoSizes` type. The values are `"default"`, `"xxsmall"`, `"xsmall"`, `"small"`,
`"medium"`, and `"large"`.

<Banner
  content={
    <>
      <strong>NOTE:</strong> <code>"default"</code> sets the width to "100%" so
      the rendered logo will expand to fill the width of the parent element.
    </>
  }
  mt="s"
  mb="s"
...
The following logos are listed in the `LogoNames` type.
...
If you'd like to pass a custom `svg` to your application, you can pass it as a
