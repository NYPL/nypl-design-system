...
After importing and implementing the `DSProvider` wrapper component, as explained in the `DSProvider` section of the DS Storybook document, your children components can use this hook function.
...
The general rule of thumb is that DS components and the available variants should be used with the styles as they are shown in Storybook, but there may be rare times when a component does require a modification. That said, if a DS component does need an updated style, first contact the Design System and UX teams about this update.

If you receive approval to make a change in a consuming app, then you can add styles directly to the `additionalStyles` prop. For example, if the Heading component should render the text in NYPL's green color used for "success primary" and add a bold font weight, then you can do the following:
