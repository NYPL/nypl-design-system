...
This is a prop that is passed to the `DSProvider` component. It is most helpful
for serverside rendering and is based on local storage and browser cookies. The
value is set through the `cookieStorageManager` if cookies are found or else the
`localStorageManager` default value.
