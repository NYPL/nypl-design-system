...
The DS spacing values are based on a root unit of `16px` (`1rem` = `16px`).
...
In consuming apps, the major page sections should be separated vertically by `2rem`. This includes the space after the page global `header` and the space before the global `footer`. Similarly, the major page sections should be separated horizontally by `2rem`.
...
When elements are displayed in a grid (i.e. Cards, Images, etc.), the space between elements, both horizontal and vertical, should be `2rem`.

|                 | Value  | JS Theme Object | CSS Variable                  |
| --------------- | ------ | --------------- | ----------------------------- |
| **Default Gap** | `1rem` | `grid.default`  | `--nypl-spacing-grid-default` |

## Form Spacing

Form input components should have a `2rem` space between each form input component. This includes both vertical (column) and horizontal (row) stacking. The DS [Form](https://nypl.github.io/nypl-design-system/storybook-static/?path=/docs/components-form-elements-form--form-with-controls) component has correct spacing incorporated into its UI structure and should always be used when building forms. Please note that the spacing in the `Form` component is identical to the spacing in the `Grid` component.

When `Checkbox` and `Radio` components are grouped together, the components should have a `1rem` space between components for vertical (column) stacking and a `2rem` space between components for horizontal (row) stacking. The DS `CheckboxGroup` and `RadioGroup` components have the correct spacing built in and should be used respectively when multiple `Checkbox` or `radio` components are being used together.

When `Button` components are grouped together, the `Button` components should have a `0.5rem` space between component. This includes both vertical (column) and horizontal (row) stacking. The DS `ButtonGroup` component has the correct spacing built in and should be used when multiple `Button` components are being used together.

|                                        | Value    | JS Theme Object              | CSS Variable                                |
| -------------------------------------- | -------- | ---------------------------- | ------------------------------------------- |
| **Default Form Gap**                   | `1rem`   | `grid.default`               | `--nypl-spacing-grid-default`               |
...
When data is displayed in a tabular layout (i.e. rows and columns in a table, a definition list, etc.), the space between data cells in a row should be `1.5rem` and the space between rows should be `2rem`.
