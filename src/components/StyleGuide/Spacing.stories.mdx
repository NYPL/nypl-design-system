...
In consuming apps, the major page sections should be separated vertically by `3rem` `(--space-xl`). This includes the gap after the page global `header` and the gap before the global `footer`.

## Grid Spacing

When elements are displayed in a grid (i.e. Cards, Images, etc.), the gutter between elements, both horizontal and vertical, should be `2rem` (`--space-l`).

## Form Spacing

Form input components should have a `2rem` (`--space-l`) gap between each component. This includes both vertical (column) and horizontal (row) stacking. The DS [Form](https://nypl.github.io/nypl-design-system/storybook-static/?path=/story/form-elements-form--form) component has correct spacing incorporated into its UI structure and should always be used when building forms.
