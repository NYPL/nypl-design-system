...
<!--
        {Object.keys(releaseparty).map((key, index) => {
          return (
            <img
              key={index}
              src={releaseparty[key]}
              width="75"
              height="75"
              alt=""
            />
        ...
        {allSvgs.map((MyComponent, index) => {
          return (
            <>
              <p>test</p>
              <MyComponent key={index} {...iconProps} />
            </>
          );
        })}
        -->
...
export const iconRow = function (array, icon, size = IconSizes.large) {
  const styles = {} as any;
...
export const UtilityIconsTemplate = () => (
