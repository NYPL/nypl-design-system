...
import { arrayToCommaList } from "../../utils/utils";
...
{ arrayToCommaList(iconRotationsArray) }.
...
type. The values are: { arrayToCommaList(iconColorsArray) }.
...
values are: { arrayToCommaList(iconSizesArray) }.
...
be found in the `IconNames` type. They are: { arrayToCommaList(iconNamesArray) }
