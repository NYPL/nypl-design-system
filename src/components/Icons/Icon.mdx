...
{<List type="ul" listItems={iconRotationsArray} />}
...
{<List type="ul" listItems={iconColorsArray.sort()} />}
...
{<List type="ul" listItems={iconSizesArray} />}
...
be found in the `IconNames` type. The values are:

{<List type="ul" listItems={iconNamesArray.sort()} />}
