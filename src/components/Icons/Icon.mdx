...
`Icon` renders commonly used icons in SVG format.
...
The `Icon`'s `aria-hidden` attribute is set to `true` by default so screenreaders
will ignore them. This is because most icons are considered decorative.

If your `Icon` is not purely decorative, pass `decorative={false}` and a
...
You may pass a `color` prop with a value from the `IconColors`.
...
You may pass `size` prop with a value from `IconSizes`.
...
The following icons are listed in the `IconNames` type.
