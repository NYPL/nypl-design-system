...
Note: There are custom props for the `Footer` component.
...
Rendering this `Footer` component, or similar HTML landmark region components,
inside another landmark region causes accessibility issues. For example,
rendering the `Footer` component inside an `aside` HTML element is an error.
