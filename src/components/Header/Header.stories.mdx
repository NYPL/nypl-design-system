...
accessibility, has static NYPL.org navigational links, a log in form for patrons
...
- `<nav>` HTML elements - The `Header` component renders one (1) `<nav>` element
  ...
  it from one another.
- Dropdowns buttons with focus-trap feature - Each dropdown button and menu
  combination uses a focus trap feature to appropriately tab through the
  contents in the menu that is opened.
...
When logged in, the dropdown menu will display the patron's name and an
