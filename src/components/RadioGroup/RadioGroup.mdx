...
Note: the width of `RadioGroup` is _not_ set to full by default. In order to make
this work, you'd need to pass the `isFullWidth` prop.
...
<Banner
  content={
    <>
      ...
      time due to an issue with forwarding the ref to Chakra's own `RadioGroup`.
      We recommend to use the controlled pattern above.
    </>
  }
  mt="s"
  mb="s"
