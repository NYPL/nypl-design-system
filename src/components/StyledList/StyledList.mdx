...
<Banner
  content={
    <>
      <strong>NOTE:</strong> either children elements or the{" "}
      <code>listItems</code> prop should be passed, but not both.
    </>
  }
  mt="s"
  mb="s"
...
an array with data can be passed into the `listItems` prop.
