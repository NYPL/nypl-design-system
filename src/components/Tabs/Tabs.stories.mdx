...
load for a user, we recommend to not use more than a combination of six (6) button
tabs and six (6) panels. Likewise, button tab labels should be clear and concise.
Do not use icons for tab labels.

The `Tabs` component is fully accessible though keyboard usage. The initial focus
...
use the `tab` key. Likewise, use the `tab + shift` key combination to more from
...
When viewing the `Tabs` component on a narrow screen, and additional carousel
