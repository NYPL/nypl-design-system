...
describe("Tabs", () => {
  ...
  it("should render nav buttons", () => {
    const tabNavs = wrapper.find(".tab-nav");
  ...
  it("should start out showing only the first tab", () => {
    ...
    const currentTab = wrapper.find(".tab-nav").at(0);
    expect(currentTab.hasClass("current")).to.be.true;
    const currentContent = wrapper.find(".tab-content").at(0);
  ...
  it("should switch tabs on click", () => {
    ...
    const button = tabNav2.find("button");
  ...
  describe("keyboard navigation", () => {
    it("should switch tabs via the right arrow key", () => {
      const button1 = wrapper.find(".tab-nav button").at(0);
      const button2 = wrapper.find(".tab-nav button").at(1);
      const button3 = wrapper.find(".tab-nav button").at(2);

      // Start out on the leftmost tab, then press the right
      // arrow key to go to the middle tab
    ...
    it("should switch tabs via the left arrow key", () => {
      ...
      // Start out on the rightmost tab, then press the left arrow key
      // to go to the middle tab
      button3.simulate("keydown", {
        keyCode: 37,
        currentTarget: { id: "2" },
      });
