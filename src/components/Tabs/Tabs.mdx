...
- It is recommended to use the `tabsData` prop to populate the component. This
...
- The carousel feature adds `"previous"` and `"next"` buttons that can be used to scroll through
...
- Horizontal scrolling can be accomplished by using the trackpad, the `shift + scroll wheel` combination, or the keyboard to tab through the available tabs.
