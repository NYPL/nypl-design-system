...
- It is recommended touse the `tabsData` prop to populate the component. This allows
  the consumer to pass in well-formed data and always generate the correct DOM.
...
<Banner
  content={
    <>
      ...
      property in each object as part of the string or as straight HTML.
    </>
  }
  type="informative"
/>
...
- Horizontal scrolling can be accomplished by using the trackpad, by using the
  `shift + scroll wheel` combination, or by using the keyboard to tab through
  the available tabs.
