...
`aria-live` determines when text updates in the component should be read to users
by assistive technologies. The `"polite"` value indicates that updates will be
announced at the next available time slot.
...
immediately. This is not recommended as it can interruptive to the user.
