...
When `aria-atomic` is set to `true` within an ARIA live region, the entire DOM
element is presented to assistive technologies. By default, `aria-atomic` is
set to `true` for the `HelperErrorText` component. If desired, you can pass
`aria-atomic={false}` instead so that only the changed node is presented.
