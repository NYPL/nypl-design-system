...
By default, `aria-atomic` is set to `true` for the `HelperErrorText` component.
This allows the entire DOM element to be presented to assistive technologies. If
desired, you can pass `aria-atomic={false}` instead so that only additions or
removals will be read by assistive technologies.
