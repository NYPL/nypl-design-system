...
- {<Link href="#html-and-jsx-children" target="_self">HTML and JSX Children</Link>}
- {<Link href="#invalid-state" target="_self">Invalid State</Link>}
...
any Chakra `Box` prop, as well as the props below.
...
The example below shows a `TextInput` component. The `TextInput` component
...
By default, this is the behavior of the `HelperErrorText` component. If desired,
...
through the `text` prop. Note that the `text` value is always wrapped in a `Box` with
