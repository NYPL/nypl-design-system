...
filter content on a page. For example, in a search results page, the filters a
user selects would be displayed back to the user through the "filter" `TagSet`
variant. These tags can be dismissed by the user and the scope of the content
should be updated accordingly -- but the update to display the proper filters
and updated results are the responsibility of the consuming application.
...
are accessible. For the "explore" variant, tags are expected to be set as links
and are therefore focusable. For the "filter" variant, tags are rendered as
buttons to perform in-page clicking behavior.
