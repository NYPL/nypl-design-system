...
- The `onClick` prop will only fire when `isDismissible` is true and the user
  clicks a tag. The function will return the entire object for the tag that was
  clicked on.
...
- When the `isDismissible` prop is true, a "Clear Filters" button will be
  ...
  trigger the `onClick` prop. The `onClick` function will return the following
  object as an argument: `{ label: "Clear Filters", id: "clear-filters" }`
...
consuming application may need for logic filtering such as `id` or `value`.
