...
<Meta
  title={getCategory("SkipNav")}
  component={SkipNav}
  parameters={{
    jest: ["SkipNav.test.tsx"],
  }}
  argTypes={{
...
<Description of={SkipNav} />

**NOTE: In order to see the links in the `SkipNav` component, start by focusing
...
<ArgsTable story="SkipNav with Controls" />

## Accessibility

To create a useable page for screen readers, a link or a navigational list of
links should be added before all the content on a page. The Reservoir achieves
this through the `SkipNav` component which renders a `<nav>` element with an
`aria-label` of "Skip Navigation". Inside the `<nav>` element, an unordered list
is rendered.
...
for more information on how to use this component with other Reservoir components.
