...
**IMPORTANT:** The `Banner` component mirrors the `Notification` component but
the `Notification` component should be used for more global messaging at the top
of the screen. The `Banner` component should be used for all messaging that
sits within the flow of the page.
...
default, when a title is passed to the `heading` prop, the
