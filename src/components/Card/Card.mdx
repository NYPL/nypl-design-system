...
The `Tooltip` relies on pointer events that are overridden by the card's `LinkOverlay`.

If you need a tooltip within a clickable `Card`, wrap the entire `Card` component inside the `Tooltip` and use the `placement` and `offset` props to move the `Tooltip` to display next to the intended reference. Keep in mind that `offset` is static pixel values, so check where the `Tooltip` displays on all breakpoints.
