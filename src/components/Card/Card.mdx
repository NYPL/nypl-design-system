...
<ComponentDocsHeader
  ...
  componentName="Card"
  summary="A flexible container used to group single-subject content in a self-
  contained format"
  versionAdded="0.24.0"
...
- [Card With Full-Click Functionality and Tooltip](#card-with-full-click-
  functionality-and-tooltip)
...
`Card` is a highly configurable and composable component. It can be shown with
or without an image and the content area is composed using Card-specific child
components: `CardHeading`, `CardActions` and `CardContent`. These child
components can be arranged in any order and each child component may be used
multiple times within the content area. Although a `Card` is a self-contained
unit, `Card` components are often grouped together and displayed in some type of
grid layout.
...
`Heading` component and accepts the [same props](../?path=/story/components-
typography-styles-heading--with-controls).
...
The `Card` composes a Chakra `Box` component, so you may pass
...
For more details, see the [Image component documentation](./?path=/docs/
components-media-icons-image--docs#fallback-image).
...
This example can be found in the [Turbine homepage](https://nypl-ds-test-app.
vercel.app/).
