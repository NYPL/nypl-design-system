...
- [Extracting Audio IDs](#extracting-audio-ids)
...
**This version will not be used for the playback of locally hosted audio files**.
...
The `VideoPlayer` component renders an `iframe` element from `Libsyn`, `SoundCloud`, or `Spotify`.
`iframe`s can be accessible if a `title` attribute is provided.
...
To guarantee that the audio player will be accessible.
...
<Canvas>
  <DSProvider>
    <AudioPlayer embedCode="<h1>embedCode</h1>" audioType="spotify" />
  </DSProvider>
</Canvas>
