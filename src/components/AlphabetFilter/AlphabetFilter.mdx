...
The `AlphabetFilter` renders a list of letter buttons, a pound sign button, and
a "Show All" (reset) button that can be used filter results. Displaying updated
results based on the user's selection is the responsibility of the consuming app.
...
The `AlphabetFilter` composes a Chakra `Box` component, so you may pass any
Chakra `Box` props, as well as the props below.
...
WAI-ARIA role of `role="navigation"`.

Each letter button has an attribute of `aria-label="Page {#}"`, where `{#}` is
equal to a button's letter value. The "Show All" button has `aria-label="Show All"`
applied. When a letter button is set to the current state (when a button is
clicked or `currentLetter` is programmatically set), `aria-current="page"` is
applied to the button and removed from all other letter buttons.
...
Use the `currentLetter` prop to programmatically set the selected letter, either
when the component first renders or at a later point.
