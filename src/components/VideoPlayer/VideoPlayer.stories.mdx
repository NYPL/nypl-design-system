...
<Meta
  ...
  argTypes={{
    ...
    useEmbed: {
      description:
        "Only used in Storybook. If `true`, set the `embedCode` prop. If `false`, set both the `videoType` and `videoId` props.",
    },
    videoType: {
      control: { type: "select" },
...
<Canvas withToolbar>
  ...
  >
    {(args) => (
      <VideoPlayer
        {...args}
        embedCode={args.useEmbed && args.embedCode}
        aspectRatio={
          !args.useEmbed && ratiosEnumValues.getValue(args.aspectRatio)
        }
        videoId={
          !args.useEmbed
        ...
        videoType={!args.useEmbed && typesEnumValues.getValue(args.videoType)}
      />
    )}
  </Story>
</Canvas>
