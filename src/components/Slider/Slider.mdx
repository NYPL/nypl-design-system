...
The `Slider` composes an HTML `input` element, so you may pass any HTML `input` attribute,
...
In addition to the `aria-label` attribute, the slider thumb will also contain
