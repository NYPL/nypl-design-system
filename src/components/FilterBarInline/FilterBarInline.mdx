...
<Source
  code={`
const { onChange, onMixedStateChange, selectedItems, onClear, onClearAll } = useFilterBar();
...
`useFilterBar` is an optional but recommended hook for cases where managing
...
handle the `selectedItems` object for `MultiSelectGroup` and `MultiSelect`
components. Included are the functions `onChange`, `onClearAll`, and
`onMixedStateChange` for handling any changes to the items that are currently
selected. Also included is the `selectedItems` object to reflect the current
state of the items that are currently selected. Additionally, the hook returns
the `setSelectedItems` function for setting an initial state of `selectedItems`
or, if needed, to manipulate `selectedItems`.
