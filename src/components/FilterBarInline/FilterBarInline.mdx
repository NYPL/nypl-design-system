...
- [FilterBarInline Heading](#filterbarinline-heading)
- [Layout Patterns](#layout-patterns)
- [UI Containers](#ui-containers)
- [Passing Children](#passing-children)
- [Controlling State Using selectedItems, onClear, and onSubmit Props](#controlling-state-using-selecteditems-onclear-and-onsubmit-props)
- [TagSet Example](#tagset-example)
- [Changelog](#changelog)

## Overview

`FilterBarInline` is a wrapper component for filter components and individual
form input components. The DS considers the following components as filter
components: `CheckboxGroup`, `MultiSelectGroup`,`RadioGroup`, `Checkbox`,
`DatePicker`, `MultiSelect`, `Radio`, `Select`, `Slider`, `TextInput`, and
`Toggle`.

The wrapped components/component groups can be displayed in a column or in a row
layout.

It is recommended to use real-time filtering in conjunction with the `TagSet`
component. If the consuming app is not using the above, then `FilterBarInline`
can render additional `Clear All` and a `Apply Filters` buttons. The two
_optional_ buttons are controlled by the `onClear` or `onSubmit` props
respectively.
...
The `FilterBarInline` component is a wrapper component for components with
filter functionality. The accessibility of `FilterBarInline` depends on the
passed child components accessibility. See the accessibility details of valid
`FilterBarInline` children components:
...
heading by passing in a DS `Heading` component in the `heading` prop.

In the following examples, the `FilterBarInline` component will render custom
`h3` and `h5` elements. Note that no matter what heading level is set, the size
will always be set to `heading5` internally by the `FilterBarInline` component.
...
The display of the `FilterBarInline` child components can be set to a `"row"` or
a `"column"` layout. On screen width under 600px width the `layout` is
...
<Banner
  content={
    <ul>
      <li>
        when <code>layout="row"</code> is passed, <code>MultiSelect</code>{" "}
        components will have <code>closeOnBlur</code> set to `true` by default
        and when
        <code>layout="column"</code> is passed, the <code>closeOnBlur</code> prop
        will be set to `false`.
      </li>
      <li>
        when <code>layout="column"</code> is passed, <code>MultiSelect</code>{" "}
        components will have
        <code>isBlockElement</code> set to <code>true</code> by default and the child
        components <code>width</code> will be set to <code>"full"</code>.
      </li>
      <li>
        <code>CheckboxGroup</code>, <code>MultiSelectGroup</code>, and{" "}
        <code>RadioGroup</code> <code>layout</code> prop will match the value of
        the <code>layout</code> set for the <code>FilterBarInline</code>{" "}
        component.
      </li>
    </ul>
  }
...
Refer to the [Passing Children](#passing-children) section for more information
on how these prop value are passed in the `renderChildren` prop.
...
<Banner
  content={
    <>
      <strong>NOTE:</strong> Future DS updates may provide a standalone
      component to implement a UI container similar to what is used in these
      examples.
    </>
  }
  mb="m"
