...
- {<Link href="#confirm-cancel-variant" target="_self">Confirm/Cancel Variant</Link>}
...
The "confirmCancel" `Modal` variant is used when the modal can confirm or revoke the modal's content. Accordingly, there are two buttons
...
<Source
  code={`
// Example for the confirmCancel variant.
modalProps={{
  type: "confirmCancel",
...
<Canvas of={ModalStories.ConfirmCancelVariant} />
