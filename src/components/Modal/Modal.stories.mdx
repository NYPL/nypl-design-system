...
import { ModalTrigger, DSModal } from "./Modal";
...
calling the `DSModal` function. This returns an object with `onOpen` and `Modal`
...
```jsx
import { DSModal } from "@nypl/design-system-react-components";

// ...
export const ModalStory = (args) => {
  const { onOpen, Modal } = DSModal();
  return (
    <>
      <ButtonGroup>
