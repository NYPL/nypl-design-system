...
Most websites have elements on that are updated dynamically. Typically, these
...
the content should be read by the assistive technology depends which of the three
...
attribute must be set to `-1` to programmatically focus on the element, and a
`focus()` method must be called on the element. The following example shows how
to move focus to an element with the `aria-live` attribute.

```html
<div class="dynamic-content-container" aria-live="polite" tabindex="-1">
  <!-- Any dynamic content should be inserted, removed, or updated here. -->
</div>
```
...
In the following example, the helper text "ErrorL Please enter a 5-digit value."
is set by default. If the user does not enter a 5-digit value, the error text
"Error: The zip code needs to be 5-digits." is displayed _and_ also announced by
assistive technology.
