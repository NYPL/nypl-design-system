...
`focus()` method must be called on the element. When focusing on an element,
the focus will automatically cause the updated content to be read and the
`aria-live` attribute should no longer be used.
...
In the following example, the helper text "Error: Please enter a 5-digit value."
