...
updated dynamically and that it should be announced. _When_ the content should
be read by the assistant technology depends on the value that is set. The
`aria-live` attribute can be set to "off", "polite", or "assertive". This
attribute does not manage focus on the element.
...
**Do not assign the `aria-live` attribute to the an entire section or the
entire page.**

Add the `aria-live` attribute to the specific element that will be updated. For
example, on an search results page that updates the amount of results found and
...
- "10 results found Title: Result 1 Author: Author 1 Title: Result 2 Author:
  Author 2 ..."
