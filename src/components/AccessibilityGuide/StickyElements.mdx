import { Meta, Source } from "@storybook/addon-docs/blocks";

import ComponentDocsHeader from "../../utils/components/ComponentDocsHeader";
import Link from "../Link/Link";

<Meta title="Accessibility Guide/Sticky Elements" />

<ComponentDocsHeader
  category="Accessibility Guide"
  componentName="Sticky Elements"
  summary="Guide to fixed position elements"
/>

## Table of Contents

- [General Information](#general-information)
- [NYPL Patterns](#NYPL-patterns)
- [General Issues](#general-issues)
- [Implementation](#implementation)
- [Resources](#resources)

## General Information

Sticky elements (`position: sticky`) switch between relative and fixed positions,
moving with the viewport as the user scrolls.

Sticky elements should be avoided. The most common use case, a sticky header, is
only useful if users are repeatedly accessing the menu and scrolling through
multiple pages, as on an e-commerce site.

Additionally, sticky headers can obscure other elements on the page. For large
menus that could cover parts of the page, consider an accordion or just making it
fixed at the top of the screen.

## NYPL Patterns

Sticky elements should generally only be used for critical information that needs
to remain visible, or to allow a continuous process for the user.

NYPL's new Digital Collections site uses a sticky header to maintain parity with
the experience of the old site.

## General Issues

When using a sticky element on a page, consider the following UX issues:

- How much space is it taking up?
- Is it difficult to access when tabbing forward and backwards?
- Does it break when users zoom in on mobile devices?
- Is it hiding a focused element?

## Implementation

The solution presented below only addresses the _last_ of the above concerns:
preventing a sticky header from hiding focused elements.

Sighted keyboard users may tab forward and backwards through a page. If an element
on a page uses `position: sticky`, tabbing backwards may result in the focused element
being behind the header, where the user can no longer see it.

The apparent solution would be using `scroll-margin-top` to snap-scroll the viewport
to wherever the focused element is, but it's not supported on several major browsers.

Instead, the following approach uses a combination of CSS, HTML, and Typescript to check
whether a focused element needs a sticky margin by dynamically calculating boundary
interactions, then applying that margin (and pushing the page up) accordingly. This React
hook is called by a focus event on any page element or a ResizeObserver event on the sticky
element.

This approach was first implemented with the Digital Collections header, using a [custom hook](https://github.com/NYPL/digital-collections/blob/main/src/hooks/useScrolled.ts)
on the header.

This code can be modified to apply to any sticky element, alongside

1. HTML data attributes on the sticky element, ex. `<header data-sticky-header> ... </header>`
2. CSS: `position: sticky` on the sticky element

The hook first defines the `sticky` object:

<Source
  code={`
  let sticky: {
    header?: HTMLElement | null;
    // Could have 'footer' as well here, for example.
    offset?: number;
  } = {};
`}
/>

`applyStickyMargin()` contains most of the logic, namely calculating boundary interactions
between the sticky object and the focused object, then applying that margin accordingly:

<Source
  code={`
  const applyStickyMargin = (sticky) => {
    // Specifying which element on the page is focused, if none then defaulting to body.
    const focused = (document.activeElement || document.body) as HTMLElement;
    let applicable = focused !== document.body;
    if (applicable && sticky.header) {
      applicable = !sticky.header.contains(focused);
    }
    // Calculating difference between the edge of the header and the focused element.
    if (applicable) {
      const edge = {
        header: sticky.header ? sticky.header.getBoundingClientRect().bottom + sticky.offset : 0,
      };
      const diff = sticky.header ? focused.getBoundingClientRect().top - edge.header : 0;
    // Then scrolls window by that difference (plus a little padding).
      if (diff < 0) {
        window.scrollBy(0, diff - 10);
      }
    }
  };
`}
  language="tsx"
/>

For the complete code and more information, check out the [hook](https://github.com/NYPL/digital-collections/blob/main/src/hooks/useScrolled.ts) and its behavior on the [Digital Collections site](https://github.com/NYPL/digital-collections).

## Resources

- [TPGi: Prevent focused elements from being obscured by sticky headers](https://www.tpgi.com/prevent-focused-elements-from-being-obscured-by-sticky-headers/)
  (This article is the source of the above approach, and this guide is pretty much an abridged summary of this article. Additionally, the code given here is Javascript and includes more sticky elements like a footer.)
- [Designing Sticky Menus: UX Guidelines](https://www.smashingmagazine.com/2023/05/sticky-menus-ux-guidelines/)
- [Sticky elements: functionality and accessibility testing](https://technology.blog.gov.uk/2018/05/21/sticky-elements-functionality-and-accessibility-testing/)
- [Sticky Elements](https://design.gs.com/patterns/sticky-elements)
