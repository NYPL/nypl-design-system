...
There's no explicit accessibility rule that dictates using an underline to
indicate a link. However, because it is a long-standing default treatment by
browsers, along with a change in color, it's arguably the most recognizable
indicator of a link's presence. In cases where it's not visually desirable
to use an underline, such as when a large number of links within body text
may negatively impact readability or scanability, then other design choices
can be used.
...
your combinations), _and_ add additional cues such as bolding, increasing the font
...
Applying a link to a full heading is the recommended pattern. When a link is applied to
a full heading:

- the link text SHOULD be rendered using the `link primary` color
- the default state of the link text SHOULD NOT be underlined
- the link text hover state SHOULD be rendered using the `link secondary`
  color and MUST be underlined

When a link is applied to a subset of words within a heading:

- the link text MUST be rendered using the `link primary` color
- the default state of the link text MUST be underlined
- the link text hover state SHOULD be rendered using the `link secondary` color
  and MUST be underlined

**While acceptable, applying a link to a portion of a heading is not the recommended
pattern.**

### Mixed Text

When a body text link sits within a block of plain (unlinked) text like a
paragraph, a single sentence, or even just a few words:
...
When a body text link sits by itself within the page content area like a "Read
More" link at the end of a short description or a "View In Catalog" link on a book
detail page:

- the link text SHOULD be rendered using the `link primary` color
- the default state of the link text SHOULD NOT be underlined
- the link text SHOULD be displayed with a "forward" directional arrow rendered using the
  `link primary` color
- the link text hover state SHOULD be rendered using the `link secondary` color and MUST be underlined
...
Links should have at least one full word (two is better). Avoid using a single
