...
[NYPL.org/accessibility](https://nypl.org/accessibility). For most digitial
applications on the NYPL.org platform, the [NYPL
Header](https://github.com/NYPL/dgx-header-component) is used and this component
...
The first link points to the `"#mainContent"` anchor which an NYPL page is
expected to have. The `TemplateAppContainer` component renders as a `main` HTML
element with a default `id` of `"mainContent"`. While it's possible to update
the target of the skip link and the id of the `<main>` element, this is not
recommended. When using the DS' `SkipNavigation` and `TemplateAppContainer`
