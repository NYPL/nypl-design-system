...
focus to an element (as in the situations described above), set the element's `tabindex` to
-1 and use the .focus() method. In React, a ref can be assigned to the element and then the
`focus()` method can be called on the ref's current property.
