...
You are expected to pass `action` and `method` attributes to specify where form
data should be sent and the method by which form will be submitted ("get" or
"post").
...
`Form` composes an HTML `form` element, so you may pass any HTML `form` attribute,
