...
The Reservoir `Button` component renders an HTML `<button>` element that is accessible.
`Button`s should be used to perform user actions or submit forms, never to link
and go to another page. For `Button`s without text and only an icon, make sure to
add an `aria-label` attribute. `Button`s cannot have images of text.
