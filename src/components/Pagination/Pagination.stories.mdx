...
### Forcing a Page Change Programatically

Generally, the selected page will change when a user clicks on a page number or
the `Previous`/`Next` buttons in the component. However, there may be circumstances
when you want to programatically change the page without the user explicitly
requesting it (for example, if you want a user to be brought back to page 1 after
entering a new search term). In this case, you can use the `currentPage` prop to
force this change. Note that the `currentPage` prop only works with the unchanging
URL `Pagination`.
...
<Canvas withToolbar>
  <Story
    name="Forcing a Page Change Programatically"
    args={{
      className: undefined,
      id: "pagination-id-3",
      initialPage: 7,
