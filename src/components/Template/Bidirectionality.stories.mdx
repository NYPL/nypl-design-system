...
supported by all majory browsers and can target specific elements through CSS rules:
...
there is no space between the text and the icon. This is because the icon has a
`margin-right` rule which works well in the default LTR display but does not apply
well in the RTL display. No audit has been made to see what components need an
update but in general, specifying an equal margin or padding to both sides of
an element may help reduce any visual mistakes.
