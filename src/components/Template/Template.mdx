...
The content area of the `Template` component (`TemplateContent`) &ndash; which
houses `TemplateContentTop`, `TemplateContentPrimary`, `TemplateContentSidebar`,
and `TemplateContentBottom` &ndash; can be configured to use the legacy layout
or to employ a responsive 12-column grid system.

Use the `useLegacyGrid` prop to configure the component accordingly.

### Legacy Layout

The legacy layout is not truely a grid system. The sidebar is set with a fixed
255px width and the primary content area expands and contracts to fill the
remain space.

### Responsive Grid System

The responsive 12-column grid configuration allows the column widths for the
sidebar and primary content to adjust depending on the current breakpoint.
Additionally, the column spacing will also adjust based on the current
breakpoint.
...
\* Primary Content width values are only applicable if the sidebar is enabled.
Otherwise, the width of the primary content area will be set to 100%.

When using the responsive 12-column grid configuration, consuming apps should
utilize `responsiveGutters` to keep their custom grids in alighment with the
spacing of the `Template` component.

<Source
  code={`
// Import the responsive spacing 
import { responsiveGutters } from "@nypl/design-system-react-components";

// Use the spacing values

<SimpleGrid spacing={responsiveSpacing}>
  <Item />
  <Item />
  <Item />
</SimpleGrid>
