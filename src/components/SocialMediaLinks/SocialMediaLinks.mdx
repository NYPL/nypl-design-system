...
import * as SocialMediaLinks from "./SocialMediaLinks.stories";
import Link from "../Link/Link";

<Meta of={SocialMediaLinks} />
...
<Description of={SocialMediaLinks} />
...
// ...
<SocialMediaLinks SocialMediaLinksData={SocialMediaLinksData} />
`}
  language="tsx"
/>
...
<Canvas of={SocialMediaLinks.WithControls} />

<ArgTypes of={SocialMediaLinks.WithControls} />

### Accessibility

Only one `SocialMediaLinks` component should be rendered on a page. This is because
only one HTML `<nav>` element with an `aria-label` attribute value of
"SocialMediaLinks" should be rendered on a page. The DS `SocialMediaLinks` component
...
- [W3 SocialMediaLinks Practice](https://www.w3.org/TR/wai-aria-practices/examples/breadcrumb/index.html)
...
<Canvas of={SocialMediaLinks.LongText} />

## Color Variations

The `SocialMediaLinks` component background color can be set to a value of the
`breadcrumbsType` type. If the `breadcrumbsType` prop is omitted, the default
background color is `"whatsOn"` (`ui.black`).

<Canvas of={SocialMediaLinks.ColorVariations} />
