...
## Default / No linksData Prop

If you pass no linksData prop to the component it returns all allowed social media types in
alphabetical order (see list below.)
In this example no props are passed at all, so the visual presentation follows all the defaults:
row layout, small size, no labels, no borders, textDefault color.
