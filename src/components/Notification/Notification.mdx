...
Note: the border you see around the `Notification` in these docs is not part of
the component. It has been added to help illustrate how the `Notification` sits
...
The `Notification` icon can be displayed with or without an icon. The default icon
can be hidden when the `showIcon` prop is set to `false`.
