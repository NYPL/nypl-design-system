...
When the `inputType` prop is set to `"textarea"`, the `<textarea>` element
...
If the `inputType` prop is set to `"email"`, `"tel"`, or `"url"`, the component will
...
`inputType` prop is set to `"email"`, `"tel"`, or `"url"`, the value of the
...
value. This clear button will render when the input has a value and disappear
...
the `inputType` prop is set to `"textarea"`.

<Banner
  mb="s"
  content={
    <>
      <strong>RECOMMENDATION:</strong> The clear button works best when the
      `TextInput` is used as an uncontrolled component.
    </>
  }
  type="recommendation"
/>
<Banner
  content={
    <>
      ...
      <List noStyling mb="0">
        <li>- the `inputType` prop is set to `"textarea"`</li>
        <li>- the `isDisabled` prop is set to `true`</li>
      </List>
    </>
  }
  type="informative"
...
`inputType="number"`. In this input type, passing a `maxLength` attribute does not
restrict the length of the number that is passed to the input. This is the
expected behavior (the native HTML input and `maxlength` attribute behave this way).

For this type, `min` and `max` HTML attributes do restrict the value of the number
input that is passed. This can be seen by using the up and down arrows to
increase or decrease the number value, respectively. However, if the `max` attribute
...
If the `min` attribute value is greater than the `max` attribute value, it will trigger
the invalid state.
...
Note that the `inputType` prop is not set to "number" in this example and that the
...
`textarea` element, pass `"textarea"` for the `inputType` prop. All the variations
