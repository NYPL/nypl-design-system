...
- {<Link href="#html-input-attributes" target="_self">HTML Input Attributes</Link>}
...
The `TextInput` props extend the native HTML input attributes so you may pass
values like `name`, `placeholder`, `value`, `step`, and `pattern`. If the input
is part of a form, you will need to include `name` for the value to be submitted
as form data.
...
`maxLength` prop or `pattern` HTML attribute. Together, these props can be set
to define how many digits and what characters are allowed. For example, if
a zip code is required, the `maxLength` prop can be set to "5" and the `pattern`
attribute can be set to `"[0-9]\*"` to allow only numbers to be entered and
