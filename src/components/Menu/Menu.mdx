...
NOTE: `labelText` is still required even if `showSelectionAsLabel` is true, because it is used as the `aria-label`, and displayed initially before the user selects an item, if `selectedItem` is not set.
