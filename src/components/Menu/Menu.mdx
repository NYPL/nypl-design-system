...
to the left of the menu text, and will render either an {<Link href="https://nypl-design-system.vercel.app/?path=/docs/components-media-icons-icon--docs"target="">Icon</Link>}
or {<Link href="https://nypl-design-system.vercel.app/?path=/docs/components-media-icons-image--docs" target="">Image</Link>}. The `media` prop requires a type (icon or image)
