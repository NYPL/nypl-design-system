...
The default label of the menu will be `labelText`. With `showSelectionAsLabel` enabled, the user's selected item will then display as the menu's label. This is useful when the user's choice
should be displayed after selection, like choosing sort/filter criteria.

NOTE: `labelText` is still required even if `showSelectionAsLabel` is true, because it is used as the `aria-label`, and displayed initially before the user selects an item.
