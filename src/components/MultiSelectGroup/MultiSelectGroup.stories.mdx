...
That includes the functions `onChange`, `onClear`, `onClearAll`, `onMixedStateChange` for handling any changes to the selection of items and the current state of the selection: `selectedItems`.
Additionally it returns `setSelectedItems` for setting an initial state of `selectedItems` or manipulate the `selectedItems` if needed.

Due to it's data structure and functions setup, it is capable to handle a group of `MultiSelect` components.
