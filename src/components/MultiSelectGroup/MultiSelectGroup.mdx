...
<Banner
  content={
    <>
      <strong>IMPORTANT:</strong> It is recommended to not set{" "}
      <code>multiSelectWidth</code> to <code>"full"</code> and{" "}
      <code>layout</code> to <code>"row"</code> at the same time. When the{" "}
      <code>MultiSelect</code> components are opened, the overlap will cause
      z-index issues with the selected count button.
    </>
  }
  mt="s"
  mb="s"
