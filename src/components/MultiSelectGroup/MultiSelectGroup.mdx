...
and the `multiSelectWidth` to "full". Note: When the screen width is less than
600px, the `layout` prop is automatically set to `"column"` and the
`multiSelectWidth` prop is set to `"full"`.
