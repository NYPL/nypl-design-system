...
- {<Link href="#feedbackbox-screens" target="_self">NewsletterSignup Screens</Link>}
...
<Source
  code={`
import { useNewsletterSignup } from "@nypl/design-system-react-components";
`}
  language="jsx"
/>
...
<Source
  code={`
const { NewsletterSignup, isOpen, onClose, onOpen } = useNewsletterSignup();
`}
  language="jsx"
/>
  ...
  return (
    <>
      <Button id="open-feedback-box" onClick={onOpen}>
        Manually Open NewsletterSignup
      </Button>
      <NewsletterSignup
        isOpen={isOpen}
        onClose={onClose}
