...
sized as an `h3. It's possible that the `h2`element will not fit in an app's heading hierarchy. If this is the case,`title`can also take a`JSX.Element`
type value.
...
let jsxHeading = <Heading level="h4">Custom h4 DS Heading</Heading>;
// ...
