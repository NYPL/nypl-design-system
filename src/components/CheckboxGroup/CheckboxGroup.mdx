...
`onChange` prop, it must be of the type listed below.
...
The `CheckboxGroup` renders a `<fieldset>` element wrapper and any `Checkbox`
children components that are passed. The `<fieldset>` element renders a
`<legend>` element that can be visually hidden through the `showLabel` prop.
...
order to make this work, set the `isFullWidth` prop to `true`.
...
checked or unchecked. In the following example, if only one child checkbox is
