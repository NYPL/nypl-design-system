...
Consuming apps may also pass an `aria-label` in on the component level:
...
When this value is set, the same `aria-label` will be applied on all accordions
in the group. Please note, however, that if an `ariaLabel` is present as a key
in the `accordionData` entries, it will override the `aria-label` passed to the
`Accordion`.
...
- An `aria-label` attribute can be applied to the accordion button using
  ...
  - Or you may set a unique value for each accordion by adding
    an `aria-label` key to each entry in the `accordionData` array
  - If you add an `aria-label` to the `Accordion` directly and to a particular
    entry, the latter will take precedence.
