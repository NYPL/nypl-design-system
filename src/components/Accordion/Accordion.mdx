...
Doing the above will be apply the `aria-label` to all accordions in the group.
Please note, however, that if an `ariaLabel` is also present as a key in
the `accordionData` entries, the key will override the `aria-label` passed to
the `Accordion`.
