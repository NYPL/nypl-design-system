...
when multiple are rendered together, they render in an "FAQ" style. When multiple
...
the necessary icon, Chakra components, and styles. Additionally, the `accordionType` can
be used to change the accordion button's background color. AccordionTypes.Default is
`ui.white`, AccordionTypes.Warning is `ui.status.primary`, and AccordionTypes.Error is
`ui.status.secondary`.
