...
**IMPORTANT**: If a custom image is passed through the `component` prop, then
`fallbackSrc` and `onError` won't work. It is expected that the custom image
that is passed implements the `onerror` attribute.
