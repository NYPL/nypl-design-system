...
The `alt` attribute should be used to concisely describe the image. If the image is purely decorative,
the `alt` attribute can be set to an empty string. In this scenario, an `alt` prop does not need to be
explicitly passed because the component will set the attribute to an empty string `""` by default.
