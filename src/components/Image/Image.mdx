...
`Image` composes an HTML `img` element, so you may pass any HTML `img` attribute, as well
any Chakra style prop and the props below.

<ArgTypes of={ImageStories.WithControls} />

## Accessibility

The `alt` attribute should be used to concisely describe the image. If the image is purely
decorative, the `alt` attribute can be set to an empty string. In this scenario, an `alt`
prop does not need to be explicitly passed because the component will set the attribute to
...
you must pass an HTML `src` attribute.

Additionally, unless the image is purely decorative, the `alt` attribute is
also required to ensure the image is accessible. If the image is purely
decorative, you may skip passing an `alt` attribute explictly as `alt=""`
