...
<Meta
  ...
  argTypes={{
    ...
    useImageWrapper: { table: { disable: true } },
  }}
/>
...
<p>
  If you want a simple HTML `img` element then set the `useImageWrapper` prop to
  `false`. This will not wrap the `img` element with styling `div` wrappers.
</p>
...
Use the `imageType` prop to get an image variant. Note that to get a perfect circle, the `imageAspectRatio` prop _must_ be set
to `ImageRatios.Square`.
