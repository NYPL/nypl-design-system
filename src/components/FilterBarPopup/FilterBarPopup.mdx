...
prop to update the `Show results` button with the total number of results associated
to the results list. If the consuming app is not using the above, then `FilterBarPopup`
can render additional `Clear All` and a `Apply Filters` buttons. The two _optional_
buttons are controlled by the `onClear` or `onSubmit` props respectively.
...
The `FilterBarPopup` component can be rendered with and without a heading. By
default, when a string is passed to the `heading` prop, the
`FilterBarPopup` component will render an `h2` element sized as a `heading5`
...
The `totalResults` prop should be used if the consuming app is utilizing real-time filtering.
If the `totalResults` prop is passed, its value will be used as part of the button's label text
(ex. "Show 72 results" where 72 is the value of the totalResults prop). This value should be updated
as filter options are selected or changed.

The example below is not fully functioning and is meant to show how the totalResults prop is shown to
users in the "Show results" button.
