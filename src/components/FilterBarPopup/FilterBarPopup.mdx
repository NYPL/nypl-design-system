...
- [FilterBarPopup Heading](#filterbarpopup-heading)
...
- [totalResults example](#totalresults-example)
- [useFilterBarPopup hook](#usefilterbarpopup-hook)
- [Changelog](#changelog)
...
prop to update the `Show X results` button with X being the total number of
results associated to the results list.

If the consuming app is not using the above, then `FilterBarPopup` will render
the `Show results` button and can render an _optional_ `Clear all filters`
...
components accessibility. See the accessibility details of valid FilterBarPopup
child components:
...
from the child form elements and restore the associated search results list to
an unfiltered state. Consuming apps are responsible for handling the logic and
subsequent UI updates expected when the `onClear` handler is called.
...
<Source
  code={`
  // function for renderChildren prop of FilterBarPopup
...
The `totalResults` prop should be used if the consuming app is utilizing
real-time filtering. If the `totalResults` prop is passed, its value will be used
as part of the label text for the `Show results` button (ex. "Show 72 results"
where 72 is the value of the totalResults prop). This value should be updated as
filter options are selected or changed.

The example below is not fully functioning and is meant to show how the
`totalResults` prop is shown to users in the `Show results` button.
...
The `useFilterBarPopup` hook provides programmatic control for the
`FilterBarPopup` component and allows consuming apps to open the modal overlay
using a custom button or methods other than a standard button click. Instead of
importing the `FilterBarPopup` component directly, use the `useFilterBarPopup`
hook to get the `FilterBarPopup` component and helper functions.
...
This hook will return an object with the `FilterBarPopup` component, a boolean
...
The `FilterBarPopup` component is the same as the one imported directly, the
`isOpen` value, and `onClose` and `onOpen` functions are exposed and available
to the consuming application. The only function that will be used directly is
the `onOpen` function. Pass `isOpen` and `onClose` to the `FilterBarPopup`.
...
The `onOpen` function can be used to programmatically open the `FilterBarPopup`
component through another element or behavior in the consuming app. See the
example below that uses a custom `Button` to open the `Modal`. The existing
button that is rendered by the `FilterBarPopup` component will still work as
expected.
