/*! For license information please see 6.da971b7200cdb8c7ed68.manager.bundle.js.LICENSE.txt */
  ...
  [6],
  {
    951: function (module, __webpack_exports__, __webpack_require__) {
      ...
      __webpack_require__.d(__webpack_exports__, "b", function () {
        return es2015_FigspecFrameViewer;
      }),
        __webpack_require__.d(__webpack_exports__, "a", function () {
          return es2015_FigspecFileViewer;
        });
      const isCEPolyfill =
      ...
      class Template {
        constructor(result, element) {
          ...
          const nodesToRemove = [],
            stack = [],
            walker = document.createTreeWalker(element.content, 133, null, !1);
          let lastPartIndex = 0,
            index = -1,
            partIndex = 0;
          const {
              ...
              if ((index++, 1 === node.nodeType)) {
                if (node.hasAttributes()) {
                  ...
                  for (let i = 0; i < length; i++)
                    endsWith(attributes[i].name, "$lit$") && count++;
                  for (; count-- > 0; ) {
                    const stringForPart = strings[partIndex],
                      name = lastAttributeNameRegex.exec(stringForPart)[2],
                      attributeLookupName = name.toLowerCase() + "$lit$",
                      attributeValue = node.getAttribute(attributeLookupName);
                ...
                if (data.indexOf(marker) >= 0) {
                  const parent = node.parentNode,
                    strings = data.split(markerRegex),
                  ...
                  for (let i = 0; i < lastIndex; i++) {
                    ...
                    if ("" === s) insert = createMarker();
                      ...
                      null !== match &&
                        endsWith(match[2], "$lit$") &&
                        (s =
                          s.slice(0, match.index) +
                          match[1] +
                          match[2].slice(0, -"$lit$".length) +
                          match[3]),
                ...
                if (node.data === marker) {
                  const parent = node.parentNode;
                  (null !== node.previousSibling && index !== lastPartIndex) ||
                    (index++, parent.insertBefore(createMarker(), node)),
                    (lastPartIndex = index),
                    this.parts.push({ type: "node", index: index }),
                    null === node.nextSibling
                      ? (node.data = "")
                      : (nodesToRemove.push(node), index--),
                    partIndex++;
                } else {
                  let i = -1;
                  for (; -1 !== (i = node.data.indexOf(marker, i + 1)); )
                    this.parts.push({ type: "node", index: -1 }), partIndex++;
                }
      ...
      const endsWith = (str, suffix) => {
        ...
        isTemplatePartActive = (part) => -1 !== part.index,
      ...
      function removeNodesFromTemplate(template, nodesToRemove) {
        const {
          ...
          walker = document.createTreeWalker(content, 133, null, !1);
        ...
        for (; walker.nextNode(); ) {
          ...
          for (
            node.previousSibling === currentRemovingNode &&
              (currentRemovingNode = null),
              nodesToRemove.has(node) &&
                (nodesToRemoveInTemplate.push(node),
                null === currentRemovingNode && (currentRemovingNode = node)),
              null !== currentRemovingNode && removeCount++;
            void 0 !== part && part.index === nodeIndex;

          )
            (part.index =
              null !== currentRemovingNode ? -1 : part.index - removeCount),
      ...
      const directives = new WeakMap(),
        isDirective = (o) => "function" == typeof o && directives.has(o),
        noChange = {},
        nothing = {};
      class template_instance_TemplateInstance {
        ...
        update(values) {
          ...
          for (const part of this.__parts)
            void 0 !== part && part.setValue(values[i]), i++;
        ...
        _clone() {
          const fragment = isCEPolyfill
            ...
            stack = [],
            parts = this.template.parts,
            walker = document.createTreeWalker(fragment, 133, null, !1);
          let part,
            partIndex = 0,
            nodeIndex = 0,
            node = walker.nextNode();
          for (; partIndex < parts.length; )
            if (((part = parts[partIndex]), isTemplatePartActive(part))) {
              for (; nodeIndex < part.index; )
                nodeIndex++,
                  "TEMPLATE" === node.nodeName &&
                    (stack.push(node), (walker.currentNode = node.content)),
                  null === (node = walker.nextNode()) &&
      ...
      class template_result_TemplateResult {
        ...
        getHTML() {
          ...
          for (let i = 0; i < l; i++) {
            const s = this.strings[i],
              commentOpen = s.lastIndexOf("\x3c!--");
            isCommentBinding =
              (commentOpen > -1 || isCommentBinding) &&
              -1 === s.indexOf("--\x3e", commentOpen + 1);
            const attributeMatch = lastAttributeNameRegex.exec(s);
            html +=
              null === attributeMatch
                ? s + (isCommentBinding ? commentMarker : nodeMarker)
        ...
        getTemplateElement() {
          ...
          return (
            void 0 !== policy && (value = policy.createHTML(value)),
      ...
      class template_result_SVGTemplateResult extends template_result_TemplateResult {
        ...
        getTemplateElement() {
          ...
          return (
            ...
            ((container, start, end = null, before = null) => {
              for (; start !== end; ) {
                const n = start.nextSibling;
                container.insertBefore(start, before), (start = n);
              }
            })(content, svgElement.firstChild),
            template
          );
        }
      }
          ...
          for (let i = 0; i < l; i++) {
            ...
            if (void 0 !== part) {
              ...
              if (isPrimitive(v) || !isIterable(v))
                text += "string" == typeof v ? v : String(v);
              else
                for (const t of v) text += "string" == typeof t ? t : String(t);
            }
          }
      ...
      class parts_NodePart {
        ...
        commit() {
          ...
          value !== noChange &&
            (isPrimitive(value)
              ...
              : value instanceof template_result_TemplateResult
        ...
        __commitText(value) {
          const node = this.startNode.nextSibling,
            valueAsString =
              "string" == typeof (value = null == value ? "" : value)
                ? value
                : String(value);
          node === this.endNode.previousSibling && 3 === node.nodeType
        ...
        __commitTemplateResult(value) {
          ...
          if (
            this.value instanceof template_instance_TemplateInstance &&
            this.value.template === template
          )
            this.value.update(value.values);
          else {
            const instance = new template_instance_TemplateInstance(
                template,
                value.processor,
                this.options
              ),
      ...
      class parts_BooleanAttributePart {
        constructor(element, name, strings) {
          if (
            ((this.value = void 0),
            (this.__pendingValue = void 0),
            2 !== strings.length || "" !== strings[0] || "" !== strings[1])
          )
            throw new Error(
      ...
      class parts_EventPart {
        ...
        commit() {
          ...
          const newListener = this.__pendingValue,
            ...
            shouldAddListener =
              null != newListener &&
              (null == oldListener || shouldRemoveListener);
          shouldRemoveListener &&
      ...
      const templateCaches = new Map(),
        render_parts = new WeakMap();
      const defaultTemplateProcessor =
        new (class default_template_processor_DefaultTemplateProcessor {
          handleAttributeExpressions(element, name, strings, options) {
            const prefix = name[0];
            if ("." === prefix) {
              return new PropertyCommitter(element, name.slice(1), strings)
                .parts;
            }
            if ("@" === prefix)
              return [
            ...
            if ("?" === prefix)
              return [
                new parts_BooleanAttributePart(element, name.slice(1), strings),
              ];
      ...
      const lit_html_html = (strings, ...values) =>
          new template_result_TemplateResult(
            strings,
            values,
            "html",
        ...
        svg = (strings, ...values) =>
          new template_result_SVGTemplateResult(
            strings,
            values,
            "svg",
            defaultTemplateProcessor
      ...
      const shadyTemplateFactory = (scopeName) => (result) => {
          ...
          const key = result.strings.join(marker);
          if (
            ((template = templateCache.keyString.get(key)), void 0 === template)
          ) {
          ...
          return (
            templateCache.stringsArray.set(result.strings, template), template
          );
        },
        ...
        prepareTemplateStyles = (scopeName, renderedDOM, template) => {
          ...
          const templateElement = template
            ...
            { length: length } = styles;
          if (0 === length)
            return void window.ShadyCSS.prepareTemplateStyles(
              templateElement,
              scopeName
            );
          ...
          template
            ? (function insertNodeIntoTemplate(template, node, refNode = null) {
                ...
                for (; walker.nextNode(); )
                  for (
                    walkerIndex++,
                      walker.currentNode === refNode &&
                        ((insertCount = countNodes(node)),
                        refNode.parentNode.insertBefore(node, refNode));
                    -1 !== partIndex && parts[partIndex].index === walkerIndex;
      ...
      class UpdatingElement extends HTMLElement {
        ...
        static createProperty(name, options = defaultPropertyDeclaration) {
          if (
            ...
            options.noAccessor || this.prototype.hasOwnProperty(name))
          )
            return;
          const key = "symbol" == typeof name ? Symbol() : `__${name}`,
            descriptor = this.getPropertyDescriptor(name, key, options);
          void 0 !== descriptor &&
        ...
        static finalize() {
          ...
          if (
            (superCtor.hasOwnProperty("finalized") || superCtor.finalize(),
            (this.finalized = !0),
            this._ensureClassProperties(),
        ...
        requestUpdateInternal(name, oldValue, options) {
          ...
          if (void 0 !== name) {
            ...
            (options = options || ctor.getPropertyOptions(name)),
              ctor._valueHasChanged(this[name], oldValue, options.hasChanged)
        ...
        requestUpdate(name, oldValue) {
          return (
            this.requestUpdateInternal(name, oldValue), this.updateComplete
          );
        }
        ...
        static _getUniqueStyles() {
          ...
          if (Array.isArray(userStyles)) {
            const addStyles = (styles, set) =>
              ...
              set = addStyles(userStyles, new Set()),
              styles = [];
      ...
      (lit_element_LitElement.finalized = !0),
        (lit_element_LitElement.render = (result, container, options) => {
          if (!options || "object" != typeof options || !options.scopeName)
            throw new Error("The `scopeName` option is required.");
          const scopeName = options.scopeName,
            hasRendered = render_parts.has(container),
            needsScoping =
              compatibleShadyCSSVersion &&
          ...
          if (
            (((result, container, options) => {
              ...
              void 0 === part &&
                ...
                render_parts.set(
                  container,
                  (part = new parts_NodePart(
                    Object.assign({ templateFactory: templateFactory }, options)
                  ))
                ),
            ...
            const template =
              part.value instanceof template_instance_TemplateInstance
                ? part.value.template
                : void 0;
...
`,
        ...
        `,
        previousStylePropertyCache = new WeakMap(),
        styleMap =
          ((f = (styleInfo) => (part) => {
            if (
              !(part instanceof parts_AttributePart) ||
            ...
            const { committer: committer } = part,
              { style: style } = committer.element;
            let previousStyleProperties = previousStylePropertyCache.get(part);
            void 0 === previousStyleProperties &&
      ...
      function stylesToArray(styles) {
        return styles ? (styles instanceof Array ? styles : [styles]) : [];
      }
      ...
      const NodeSelectableMixin = (superClass) => {
        ...
        return (
          __decorate(
            [decorators_property({ attribute: !1 })],
      ...
      function shouldSkipEvent(ev) {
        return 0 === ev.touches.length || ev.touches.length > 2;
      }
      const TouchGestureMixin = (superClass) =>
          ...
          constructor(...args) {
            super(...args),
              ...
              this.addEventListener("touchstart", (ev) => {
                shouldSkipEvent(ev) ||
                  (ev.preventDefault(), (this.previousTouches = ev.touches));
              }),
              this.addEventListener("touchend", (ev) => {
                shouldSkipEvent(ev) ||
                  (ev.preventDefault(), (this.previousTouches = null));
              }),
              ...
              this.addEventListener("touchmove", (ev) => {
                ...
                const previousTouches = Array.from(this.previousTouches || []),
                  currentTouches = Array.from(ev.touches);
                (this.previousTouches = ev.touches),
                  currentTouches.length === previousTouches.length &&
                    currentTouches.every((t) =>
                      previousTouches.some(
                        (pt) => pt.identifier === t.identifier
                      )
                    ) &&
                    (1 !== currentTouches.length
                      ? this.onTouchPinch(
      ...
      var PositionedMixin_decorate = function (decorators, target, key, desc) {
          var d,
            c = arguments.length,
            r =
              c < 3
      ...
      const PositionedMixin = (superClass) => {
          ...
          class Positioned extends TouchGestureMixin(superClass) {
            constructor(...args) {
              super(...args),
                ...
                (this.zoomSpeed = 500),
                (this.panSpeed = 500),
                _isDragModeOn.set(this, !1),
                _movePanel.set(this, (shiftX, shiftY) => {
                  (this.panX += shiftX / this.scale / window.devicePixelRatio),
                    (this.panY +=
                      shiftY / this.scale / window.devicePixelRatio);
                }),
                _keyDown.set(this, (event) => {
                  "Space" !== event.code ||
                    __classPrivateFieldGet(this, _isDragModeOn) ||
                    (__classPrivateFieldSet(this, _isDragModeOn, !0),
                    (document.body.style.cursor = "grab"));
                }),
                _keyUp.set(this, (event) => {
                  "Space" === event.code &&
                    __classPrivateFieldGet(this, _isDragModeOn) &&
                    (__classPrivateFieldSet(this, _isDragModeOn, !1),
                    (document.body.style.cursor = "auto"));
                }),
                _listenToKeyboardEvents.set(this, () => {
                  document.addEventListener(
                    "keyup",
                    __classPrivateFieldGet(this, _keyUp)
                  ),
                    document.addEventListener(
                      "keydown",
                      __classPrivateFieldGet(this, _keyDown)
                    );
                }),
                  ...
                  (ev) => {
                    if (this.isMovable)
                      if ((ev.preventDefault(), ev.ctrlKey)) {
                        let { deltaY: deltaY } = ev;
                        1 === ev.deltaMode && (deltaY *= 15);
                        const prevScale = this.scale;
                        this.scale *=
                          1 - deltaY / (0.5 * (1e3 - this.zoomSpeed));
                        const offsetX = ev.offsetX - this.offsetWidth / 2,
                          offsetY = ev.offsetY - this.offsetHeight / 2;
                        (this.panX +=
                          offsetX / this.scale - offsetX / prevScale),
                          (this.panY +=
                            offsetY / this.scale - offsetY / prevScale);
                      } else {
                        const speed = 0.002 * this.panSpeed;
              ...
              this.addEventListener("gesturestart", (ev) => {
                ...
                this.addEventListener("pointermove", (ev) => {
                  4 & ev.buttons &&
                    (ev.preventDefault(),
                    __classPrivateFieldGet(this, _movePanel).call(
                      this,
                      ev.movementX,
                      ev.movementY
                    ));
                }),
                __classPrivateFieldGet(this, _listenToKeyboardEvents).call(
                  this
                ),
                (this.onmousedown = () => {
                  __classPrivateFieldGet(this, _isDragModeOn) &&
            ...
            updated(changedProperties) {
              super.updated(changedProperties),
                ...
                (changedProperties.has("panX") ||
                  changedProperties.has("panY")) &&
                  this.dispatchEvent(
          ...
          return (
            (_isDragModeOn = new WeakMap()),
            (_movePanel = new WeakMap()),
            (_keyDown = new WeakMap()),
            (_keyUp = new WeakMap()),
            (_listenToKeyboardEvents = new WeakMap()),
            ...
            PositionedMixin_decorate(
              [decorators_property({ type: Number, attribute: "zoom-speed" })],
              Positioned.prototype,
              "zoomSpeed",
              void 0
            ),
            PositionedMixin_decorate(
              [decorators_property({ type: Number, attribute: "pan-speed" })],
              Positioned.prototype,
              "panSpeed",
              void 0
            ),
        ...
        Guides = ({
          ...
          return (
            guides ||
              ((guides = (function getDistanceGuides(selected, compared) {
                const a = absRect(selected),
                  b = absRect(compared),
                  isYIntersecting = !(a.top > b.bottom || a.bottom < b.top),
                  isXIntersecting = !(a.left > b.right || a.right < b.left);
                if (isXIntersecting && isYIntersecting) {
                  ...
                  return [
                    ...
                    {
                      points: [
                        { y: a.bottom, x: intersectCenter.x },
              ...
              })(node.absoluteBoundingBox, distanceTo.absoluteBoundingBox)),
              guidesCache.set(combinedId, guides)),
            [
              ...guides.map((guide) =>
                (({ guide: guide, reverseScale: reverseScale }) => {
                  const xLength = Math.abs(
                      guide.points[0].x - guide.points[1].x
                    ),
                    yLength = Math.abs(guide.points[0].y - guide.points[1].y);
                  return 0 === xLength && 0 === yLength
                    ? null
                    : svg`
    <line
      class="distance-line"
              ...
              ...guides.map((guide) =>
                (({
                  ...
                  const xLength = Math.abs(
                    ...
                    yLength = Math.abs(guide.points[0].y - guide.points[1].y);
                  if (0 === xLength && 0 === yLength) return null;
                  const text = round(Math.max(xLength, yLength)).toString(10),
                    width = text.length * fontSize * 0.5,
                    startMargin = 0.25 * fontSize,
                    vPadding = 0.25 * fontSize,
                    hPadding = 0.5 * fontSize,
                    x =
                      xLength > yLength
                        ? (guide.points[0].x + guide.points[1].x) / 2 -
                          width / 2
                        : guide.points[0].x,
                    y =
                      xLength > yLength
                        ? guide.points[0].y
                        : (guide.points[0].y + guide.points[1].y) / 2 -
                          fontSize / 2,
                    transform = [
                      `scale(${reverseScale})`,
                      xLength > yLength
  ...
  `;
        ...
        `;
      var copy_to_clipboard = __webpack_require__(431);
      const CopyIcon = ({ onClick: onClick = () => {} }) => svg`
...
`,
        extractColorStyle = (color) =>
          0 === color.a
            ? "transparent"
            : color.a < 1
            ? `rgba(${rgbToIntArray(color).join(", ")}, ${color.a.toFixed(2)})`
            : rgbToHex(color);
      class Gradient {
        constructor(data) {
        ...
        get cssGradientArray() {
          return this.colorObjects.map(
            (color, index) =>
              color + " " + this.floatToPercent(this.colors[index].position)
          );
        }
        get cssColor() {
          ...
          return (
            ...
            `linear-gradient(${cssGradientArray.join(", ")})`
          );
        }
        createColorObjects(colors) {
          return colors.map(({ color: color }) => extractColorStyle(color));
        }
        ...
        calculateAngle(startHandle, endHandle) {
          const radians = Math.atan(
            this.calculateGradient(startHandle, endHandle)
          );
          return parseInt(this.radToDeg(radians).toFixed(1));
        }
        calculateGradient(startHandle, endHandle) {
          return (
            ((endHandle.y - startHandle.y) / (endHandle.x - startHandle.x)) * -1
          );
        }
        radToDeg(radian) {
          ...
          if (
            ...
            node.style &&
              ...
              (this.fontPostScriptName =
                null === (_a = node.style.fontPostScriptName) || void 0 === _a
                  ? void 0
                  : _a.replace("-", " ")),
            ...
            node.backgroundColor || node.backgroundColor)
          ) {
            const color =
              node.backgroundColor ||
              (null === (_b = node.background) || void 0 === _b
                ? void 0
                : _b[0].color);
            this.background = extractColorStyle(color);
          }
          const fillColor =
            null === (_c = node.fills) || void 0 === _c ? void 0 : _c[0];
          if (
            (fillColor &&
              !1 !== fillColor.visible &&
              ("TEXT" === node.type
                ? (this.color = extractColorStyle(fillColor.color))
                : fillColor.type.includes("GRADIENT")
                ? (this.backgroundImage = new Gradient(fillColor).cssColor)
                : "SOLID" === fillColor.type &&
            ...
            (this.boxShadowColor = extractColorStyle(color)),
              (this.boxShadow = `${
                (null == offset ? void 0 : offset.x) || 0
              }px ${(null == offset ? void 0 : offset.y) || 0}px 0 ${radius} ${
                this.boxShadowColor
              }`);
          }
        }
      ...
      const rgbToIntArray = (color) => [
        ...
        getStyleRule = ({ property: property, value: value }) =>
          `${property}: ${value};`,
        View = ({ node: node, onClose: onClose }) => {
          if (!node) return null;
...
`} ${nodeStyles.verticalPadding}
  ...
  `;
    ...
    <div class="title-section style-section">
      <h4>CSS</h4>
      ${CopyIcon({
        onClick: () => copy_to_clipboard(nodeStyles.getStyleSheet()),
      })}
    </div>
    <div class="code-section selectable-content">
      ...
      ></span>`;
          ...
          return lit_html_html`<div class="css-property" @click=${() =>
            copy_to_clipboard(getStyleRule(cssProperty))}>
    <span>${property}:</span>${coloredSquare}<span class="css-value">${value}</span>;</span>
  </div>`;
        },
        InspectorView_styles = css`
          .inspector-view {
            height: 100%;
            width: 300px;
            position: absolute;
        ...
        Outline = ({
          node: node,
          selected: selected = !1,
          computedThickness: computedThickness,
          onClick: onClick,
        }) => {
          const {
              x: x,
            ...
            shift = computedThickness / 2,
            lineTo = (x, y) => `L${x},${y}`,
            arcTo = (r, x, y) => `A${r},${r} 0 0 1 ${x},${y}`,
            boxPath = [
              ((x, y) => `M${x},${y}`)(radius.topLeft + shift, shift),
              ...
              arcTo(
                radius.bottomLeft - shift,
                shift,
                height - radius.bottomLeft
              ),
     ...
     */
        ...
        Footer = (metadata) => {
          if (
            !metadata ||
            !metadata.link ||
            void 0 === metadata.link ||
            "undefined" === metadata.link
          )
            return null;
          const {
            link: link,
...
`} </span>
    ...
    >
      Edited ${((
        date,
        nowDate = Date.now(),
        rft = new Intl.RelativeTimeFormat(void 0, { numeric: "auto" })
      ) => {
        const diff = getTime(nowDate) - getTime(date),
      ...
      var ViewerMixin_decorate = function (decorators, target, key, desc) {
          var d,
            c = arguments.length,
            r =
              c < 3
        ...
        ViewerMixin_classPrivateFieldGet = function (receiver, privateMap) {
          if (!privateMap.has(receiver))
            throw new TypeError(
              "attempted to get private field on non-instance"
            );
        ...
        ViewerMixin_classPrivateFieldSet = function (
          receiver,
          privateMap,
          value
        ) {
      ...
      const ViewerMixin = (superClass) => {
        ...
        class Viewer extends NodeSelectableMixin(PositionedMixin(superClass)) {
            ...
            super(...args),
              ...
              _getNodeById.set(this, (id) => {
                ...
                return null !==
                  (_b =
                    null ===
                      (_a = ViewerMixin_classPrivateFieldGet(
                        this,
                        _flattenedNodes
                      )) || void 0 === _a
                      ? void 0
                      : _a.find((n) => n.id === id)) && void 0 !== _b
                  ? _b
                  : null;
              });
          }
            ...
            return extendStyles(super.styles, [
              css`
                :host {
                  ...
                  --error-bg: var(
                    --figspec-viewer-error-bg,
                    var(--default-error-bg)
                  );
                  --error-fg: var(
                    --figspec-viewer-error-fg,
                    var(--default-error-fg)
                  );
                  --error-color: var(--figspec-viewer-error-color, tomato);

                  --guide-thickness: var(
                    --figspec-viewer-guide-thickness,
                    1.5px
                  );
                  ...
                  --guide-tooltip-fg: var(
                    --figspec-viewer-guide-tooltip-fg,
                    white
                  );
          ...
          get error() {
            return ViewerMixin_classPrivateFieldGet(this, _canvasSize) &&
              ViewerMixin_classPrivateFieldGet(this, _flattenedNodes)
              ? null
              : ErrorMessage({
                  title: "Error",
                  children:
          ...
          render() {
            if (this.error)
              return this.error instanceof Error
                ...
                ? ErrorMessage({ title: "Error", children: this.error })
                : this.error;
            const canvasSize = ViewerMixin_classPrivateFieldGet(
                this,
                _canvasSize
              ),
              reverseScale = 1 / this.scale,
              guideThickness = `calc(var(--guide-thickness) * ${reverseScale})`,
              computedGuideThickness = parseFloat(
                getComputedStyle(this).getPropertyValue("--guide-thickness")
              ),
              computedGuideTooltipFontSize = parseFloat(
        ...
        "
          >
            ${Object.entries(this.__images).map(([nodeId, uri]) => {
              var _a;
              const node = ViewerMixin_classPrivateFieldGet(
                this,
                _getNodeById
              ).call(this, nodeId);
              if (
                ...
                return null;
              const margin = ViewerMixin_classPrivateFieldGet(
                this,
                _effectMargins
              )[node.id];
              return lit_html_html`
            ...
            ${
              this.selectedNode &&
              (({
                nodeSize: { x: x, y: y, width: width, height: height },
                offsetX: offsetX,
                offsetY: offsetY,
                reverseScale: reverseScale,
              }) => lit_html_html`
    <div class="tooltip" style="${styleMap({
  ...
  `)({
            ...
            >
              ...
              ${ViewerMixin_classPrivateFieldGet(this, _flattenedNodes).map(
                (node) => {
                  var _a;
                  return node.id ===
                    (null === (_a = this.selectedNode) || void 0 === _a
                      ? void 0
                      : _a.id)
                    ? null
                    : svg`
                  <g>
                    ${Outline({
                      node: node,
                      computedThickness: computedGuideThickness * reverseScale,
                      onClick: ViewerMixin_classPrivateFieldGet(
                        this,
                        _handleNodeClick
                      ).call(this, node),
                    })}
      ...
      `;
              ...
              "CANVAS" === node.type
                ? (function getCanvasSize(node) {
                    ...
                    for (const child of node.children) {
                      if ("FRAME" !== child.type && "COMPONENT" !== child.type)
                        continue;
                      const {
                        x: x,
                        y: y,
          ...
          __updateEffectMargins() {
            ...
            ViewerMixin_classPrivateFieldSet(
              ...
              containers.reduce(
                (margin, node) =>
                  "absoluteBoundingBox" in node
                    ? Object.assign(Object.assign({}, margin), {
          ...
          resetZoom() {
            if (ViewerMixin_classPrivateFieldGet(this, _canvasSize)) {
              const { width: width, height: height } =
                  ViewerMixin_classPrivateFieldGet(this, _canvasSize),
                { width: elementWidth, height: elementHeight } =
                  this.getBoundingClientRect(),
                wDiff = elementWidth / (width + 2 * this.zoomMargin),
                hDiff = elementHeight / (height + 2 * this.zoomMargin);
        ...
        return (
          ...
          (_effectMargins = new WeakMap()),
          (_flattenedNodes = new WeakMap()),
          (_handleNodeClick = new WeakMap()),
          (_getNodeById = new WeakMap()),
          ViewerMixin_decorate(
            [decorators_property({ type: Number, attribute: "zoom-margin" })],
            Viewer.prototype,
            "zoomMargin",
            void 0
          ),
          ViewerMixin_decorate(
            [decorators_property({ type: String, attribute: "link" })],
            Viewer.prototype,
            "link",
            void 0
          ),
      ...
      function getEffectMargin(container, nodes) {
        const points = nodes.map((node) => {
            ...
            const blurRadiuses = node.effects
                .filter(
                  (effect) => effect.visible && "LAYER_BLUR" === effect.type
                )
                .map((effect) => effect.radius),
              shadowMargins = node.effects
                .filter(
                  (effect) =>
                    effect.visible &&
                    "DROP_SHADOW" === effect.type &&
                    !!effect.offset
                )
                .map((effect) => ({
            ...
            return {
              top: node.absoluteBoundingBox.y - margin_top,
              right:
              ...
              bottom:
                ...
                node.absoluteBoundingBox.height +
                margin_bottom,
              left: node.absoluteBoundingBox.x - margin_left,
            };
          }),
          bounds_top = Math.min(...points.map((p) => p.top)),
          bounds_right = Math.max(...points.map((p) => p.right)),
          bounds_bottom = Math.max(...points.map((p) => p.bottom)),
          bounds_left = Math.min(...points.map((p) => p.left));
        return {
          top: container.absoluteBoundingBox.y - bounds_top,
          right:
            bounds_right -
          ...
          left: container.absoluteBoundingBox.x - bounds_left,
        };
      }
      function flattenNode(node, depth = 0) {
        return "absoluteBoundingBox" in node
          ? "children" in node && 0 !== node.children.length
            ? [
                Object.assign(Object.assign({}, node), { depth: depth }),
                ...node.children
                  .map((child) => flattenNode(child, depth + 1))
                  .flat(),
              ]
            : [Object.assign(Object.assign({}, node), { depth: depth })]
          : node.children.map((child) => flattenNode(child, depth + 1)).flat();
      }
      var FigspecFrameViewer_decorate = function (
        decorators,
        target,
        key,
        desc
      ...
      class FigspecFrameViewer_FigspecFrameViewer extends ViewerMixin(
        ...
        get documentNode() {
          ...
          return documentNode && "absoluteBoundingBox" in documentNode.document
        ...
        get error() {
          return this.nodes && this.renderedImage
            ? this.documentNode
              ? super.error
                ? super.error
                : void 0
        ...
        updated(changedProperties) {
          if (
            (super.updated(changedProperties), changedProperties.has("nodes"))
          ) {
            if (!this.documentNode) return;
            this.__updateTree(this.documentNode), this.resetZoom();
          }
          changedProperties.has("renderedImage") &&
            this.__updateEffectMargins();
        }
      }
      FigspecFrameViewer_decorate(
        [decorators_property({ type: Object })],
        FigspecFrameViewer_FigspecFrameViewer.prototype,
        "nodes",
        void 0
      ),
        FigspecFrameViewer_decorate(
          [decorators_property({ type: String, attribute: "rendered-image" })],
          FigspecFrameViewer_FigspecFrameViewer.prototype,
          "renderedImage",
          void 0
        );
      var _selectFirstPage,
        _handlePageChange,
        FigspecFileViewer_decorate = function (decorators, target, key, desc) {
          var d,
            c = arguments.length,
            r =
              c < 3
      ...
      class FigspecFileViewer_FigspecFileViewer extends ViewerMixin(
        lit_element_LitElement
      ) {
        constructor() {
          super(...arguments),
            ...
            _selectFirstPage.set(this, () => {
              ...
              this.documentNode
                ? (this.selectedPage =
                    null !==
                      (_a = this.documentNode.document.children.filter(
                        (c) => "CANVAS" === c.type
                      )[0]) && void 0 !== _a
                      ? _a
                      : null)
            ...
            _handlePageChange.set(this, (ev) => {
              ...
              (this.selectedPage =
                null !==
                  (_b =
                    null === (_a = this.documentNode) || void 0 === _a
                      ? void 0
                      : _a.document.children.find(
                          (c) => c.id === target.value
                        )) && void 0 !== _b
                  ? _b
                  : null),
        ...
        get error() {
          return this.documentNode && this.renderedImages
            ? super.error
              ? super.error
              : void 0
            : ErrorMessage({
        ...
        render() {
          ...
          return lit_html_html`
      <div class="controls">
        <select @change=${FigspecFileViewer_classPrivateFieldGet(
          this,
          _handlePageChange
        )}>
          ${
            null === (_a = this.documentNode) || void 0 === _a
              ? void 0
              : _a.document.children.map(
                  (c) => lit_html_html`<option value=${c.id}>${c.name}</option>`
                )
          }
        </select>
      </div>
    ...
    `;
        ...
        updated(changedProperties) {
          super.updated(changedProperties),
            changedProperties.has("documentNode") &&
              (FigspecFileViewer_classPrivateFieldGet(
                this,
                _selectFirstPage
              ).call(this),
              this.selectedPage &&
                (this.__updateTree(this.selectedPage), this.resetZoom())),
            changedProperties.has("renderedImages") &&
              this.__updateEffectMargins();
        }
      }
      (_selectFirstPage = new WeakMap()),
        (_handlePageChange = new WeakMap()),
        FigspecFileViewer_decorate(
          [decorators_property({ type: Object, attribute: "document-node" })],
          FigspecFileViewer_FigspecFileViewer.prototype,
          "documentNode",
          void 0
        ),
        FigspecFileViewer_decorate(
          [decorators_property({ type: Object, attribute: "rendered-images" })],
          FigspecFileViewer_FigspecFileViewer.prototype,
          "renderedImages",
          void 0
        ),
        customElements.get("figspec-file-viewer") ||
          customElements.define(
            "figspec-file-viewer",
            FigspecFileViewer_FigspecFileViewer
          ),
        customElements.get("figspec-frame-viewer") ||
          customElements.define(
            "figspec-frame-viewer",
            FigspecFrameViewer_FigspecFrameViewer
          );
      ...
      const bindEvent = (element, event, cb) => {
        ...
        es2015_FigspecFrameViewer = Object(react.forwardRef)((_a, ref) => {
          var {
              nodes: nodes,
        ...
        es2015_FigspecFileViewer = Object(react.forwardRef)((_a, ref) => {
          var {
    ...
    956: function (module, __webpack_exports__, __webpack_require__) {
      ...
      __webpack_require__.r(__webpack_exports__),
        function (process) {
          __webpack_require__.d(__webpack_exports__, "Figspec", function () {
            return Figspec;
          });
          var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(0),
            _figspec_react__WEBPACK_IMPORTED_MODULE_1__ =
              __webpack_require__(951),
            _storybook_components__WEBPACK_IMPORTED_MODULE_2__ =
              __webpack_require__(316),
            _storybook_theming__WEBPACK_IMPORTED_MODULE_3__ =
              __webpack_require__(42),
            _Figma__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(435),
          ...
          var templateObject_1,
            Figspec = function (_a) {
              var config = _a.config,
                _b = Object(react__WEBPACK_IMPORTED_MODULE_0__.useState)({
                  state: "loading",
                }),
                state = _b[0],
                setState = _b[1],
                fetchDetails = function (signal) {
                  return __awaiter(void 0, void 0, void 0, function () {
                    ...
                    return __generator(this, function (_b) {
                      switch (_b.label) {
                        case 0:
                          setState({ state: "loading" }), (_b.label = 1);
                        case 1:
                          if (
                            (_b.trys.push([1, 6, , 7]),
                            !(match = config.url.match(
                              _Figma__WEBPACK_IMPORTED_MODULE_4__.b
                            )))
                          )
                            throw new Error(
                              config.url + " is not a valid Figma URL."
                            );
                          if (
                            ((fileKey = match[3]),
                            (url = new URL(config.url)),
                            (nodeId = url.searchParams.get("node-id")),
                            (accessToken = (function getAccessToken(cfg) {
                              var _a;
                        ...
                        case 6:
                          return (err_1 = _b.sent()) instanceof DOMException &&
                            err_1.code === DOMException.ABORT_ERR
                            ? [2]
                            : (console.error(err_1),
                              setState({
                                state: "failed",
              ...
              switch (
                ...
                case "loading":
                  return Object(
                    ...
                    _storybook_components__WEBPACK_IMPORTED_MODULE_2__.a,
                    null,
                    Object(_storybook_theming__WEBPACK_IMPORTED_MODULE_3__.d)(
                ...
                case "fetched":
                  return "file" === state.value.type
                    ? Object(_storybook_theming__WEBPACK_IMPORTED_MODULE_3__.d)(
                        _figspec_react__WEBPACK_IMPORTED_MODULE_1__.a,
                        __assign({ css: fullscreen }, state.value.props)
                      )
                    : Object(_storybook_theming__WEBPACK_IMPORTED_MODULE_3__.d)(
                        _figspec_react__WEBPACK_IMPORTED_MODULE_1__.b,
                        __assign({ css: fullscreen }, state.value.props)
                      );
              }
            };
          function listAllFrames(node) {
            return "absoluteBoundingBox" in node
              ...
              ? node.children.map(listAllFrames).flat()
              : [];
          }
          __webpack_exports__.default = Figspec;
